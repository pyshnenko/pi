	...
	if ((ctx.message.text === '_открыть шлагбаум_')&&((admin.includes(ctx.from.id))||(notRoot.includes(ctx.from.id)))) {
		...
		if ((Number(new Date()) - Number(new Date(ctx.message.date*1000)))<20000) {
			if (door) ctx.reply('занято. подожди 10 секунд');
				...
				ctx.reply('открываю');
				//ctx.reply('Если не открылось, приношу свои извинения. Из-за погодных условий радиосигнал не всегда доходит до шлагбаума. Постараюсь исправить. Воспользуйтесь другим способом открытия');
				door=1;
